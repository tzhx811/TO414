...
```{r}
set.seed(12345)
testrows_knn <- sample(1:nrow(stroke_norm), 1200) 
...
stroke_knn_test_pred <- knn(train = stroketrain_knn, test = stroketest_knn,
                      cl = knn_train_labels, k=sqrt(nrow(stroketrain_knn)))
```

```{r}
summary(stroke_knn_test_pred)
library(gmodels)
CrossTable(x = knn_test_labels, y = stroke_knn_test_pred, 
           prop.chisq=FALSE)
```