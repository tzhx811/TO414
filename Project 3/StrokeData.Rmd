...
Our general KNN model using max-min normalization has a kappa value of 0.61, which is significantly higher than other individual models.The accuracy of the model is 97.18% overall, and out of 66 patients who do have stroke, our model predicted 36 of them correctly (54.5% accuracy). 
...
After splitting the data by age, we build 2 separate models and put them together for the combined model. As seen in the confusion matrices, the kappa value for the young-patients knn model is only 0.22, while it is 0.76 for old-patients data. This is becuase there are only 8 young patients in our test data actually have stroke, while 61 old patients have stroke. As result, the KNN model is very powerful when predicting whether an old patient would have stroke.
The combined model results have a kappa value of 0.7348, which is slightly lower than the model of old patients data. But it is still improved from the original model without the age split.
...
After splitting the data by bmi value, we build 2 separate models and put them together for the combined model. As seen in the confusion matrices, the kappa value for the risky group (bmi value over 26.5) knn model is only 0.3867, and it is 0.5964 for safe group. As result, our KNN model is more powerful when predicting results for patients with bmi balue lower than 26.5.
When we combine two models' resultgs together, the combined model results have a kappa value of 0.4927, which is lower than the kappa values of both the original KNN model and the age split KNN model.

Moving forward, the KNN models have pretty significant kappa values and should be considered as an important factor in the final stacked model. Moreover, the KNN model built on age-split data is the most powerful knn model with kappa value over 0.7. 
...
Our general ANN model does a poor job in predicting stroke for patients. It has a kappa value of 0.025, which is significantly lower than other individual models.The accuracy of the model is 94.75% overall, and out of 66 patients who do have stroke, our model predicted only 1 of them correctly. This is worrying, especially with so many false negatives. 
...
While the overall accuracy (94.52%) took a slight dip, splitting by Age improved the ANN model marginally. Now, the Kappa statistic has increased to 0.09. While that still is not a very assuring number, it is an improvement from the non-split ANN model. Before, only 1 correct positive stroke prediction was made. Now, 4 patients were correctly predicted to have a stroke. 
...
Similar to the previous Age-based model, the BMI-based ANN Model slightly increases the Kappa statistic (0.0781) while slightly decreasing the overall accuracy (94.21%). These figures still are not promising, given that only 4 strokes were accurately predicted. This is the same amount of correctly predicted strokes as the Age-based ANN model.

Overall, it seems like the ANN model is not the most useful for stroke predictions, even after splitting by both BMI and Age. The kappa statistics for the general model, Age-based model, and BMI-based model are all far too low to deem these models as valuable. 
...
```
Based on the stacked model, it seems that the model is mainly making its decisions off of the decision tree and KNN model, but mostly the KNN model. However, even with all the models' predictions running as variables, the kappa and accuracy statistic of the stacked model does not out perform that of the KNN model or logistic regression model performing individually. The kappa statistic of the stacked model is comparable to the individual performance of the decision, doing better than the ANN and SVM model. 
...
```
We see a similar result from the BMI stacked model. This is probably due to similar reasons outlined above from the age stacked model. 
