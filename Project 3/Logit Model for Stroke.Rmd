...
#logit_finalCost <- glm(stroke ~ +age +avg_glucose_level  +work_typeSelf.employed +avg_glucose_level , data = stroketrainLOG, costs = error_cost, family = "binomial")
#summary(logit_finalCost)



#LogCostPredict <- predict(logit_finalCost, stroketrainLOG)
#CrossTable(x = stroketestLOG$stroke, y = LogCostPredict, prop.chisq=FALSE)
#confusionMatrix(data = as.factor(LogCostPredict), reference = as.factor(stroketestLOG$stroke)
