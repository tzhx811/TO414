...
finallogmodel_young <- glm(stroke ~ +age +hypertension, data = age_young_data_norm[[1]], family = "binomial")
summary(finallogmodel_young)

young_Logpred <- predict(finallogmodel_young, age_young_data_norm[[2]])
...
finallogmodel_old <- glm(stroke ~ +age +avg_glucose_level +work_typePrivate, data = age_old_data_norm[[1]], family = "binomial")
summary(finallogmodel_old)

old_Logpred <- predict(finallogmodel_old, age_old_data_norm[[2]])
...
confusionMatrix(as.factor(combined_Logpred), as.factor(age_true_label))
summary(young_Logpred)
summary(old_Logpred)
summary(combined_Logpred)
```
...
finallogmodel_risk <- glm(stroke ~ +age +avg_glucose_level, data = bmi_risk_data_norm[[1]], family = "binomial")
summary(finallogmodel_risk)

risk_Logpred <- predict(finallogmodel_risk, bmi_risk_data_norm[[2]])
risk_Logpred = ifelse(risk_Logpred>0, 0, 1)

#Safe
masslogmodel_safe <- glm(stroke ~., data = bmi_safe_data_norm[[1]], family = "binomial")
summary(masslogmodel_safe)

finallogmodel_safe <- glm(stroke ~ +age +avg_glucose_level +work_typePrivate, data = age_old_data_norm[[1]], family = "binomial")
summary(finallogmodel_safe)

safe_Logpred <- predict(finallogmodel_safe, bmi_safe_data_norm[[2]])
safe_Logpred = ifelse(safe_Logpred>0, 0, 1)
...
confusionMatrix(as.factor(combined_Logpred), as.factor(bmi_true_label))
summary(risk_Logpred)
summary(safe_Logpred)
summary(bmi_combined_Logpred)
